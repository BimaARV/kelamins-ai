...
async def _reply_ai(text: str, chat_id: str, message_id: int) -> None:
    ...
    system = (
        ...
        "pakai notasi LaTeX (\\$, \\rightarrow, \\frac, dst) dan JANGAN nulis tag HTML "
        "(\\<b\\>, \\<i\\>, \\<span style=...\\>, \\<br\\>) — dua-duanya nggak ke-render, "
        "malah jadi teks mentah. Format cuma pakai markdown: **tebal**, *miring*, '- list', "
        "backtick buat kode, '→' buat arah. Pakai panah "
        "'→' atau kata 'ke'/'menuju'; jawaban panjang pecah per poin dengan **teks "
